...
/**
 * ProxyMethod is compiled by ProxyManager and attached to ProxyType to define a
 * method that is proxied.
 * 
...
public class ProxyMethod implements JsonSerializable {
  ...
  public ProxyMethod(Method method) {
    ...
    if (returnType.isArray() || Iterable.class.isAssignableFrom(returnType) || isMap) {
      // How to present on the client - only ArrayList by default is wrapped on the
      // client
      Remote.Array array;
  ...
  public Object getPrefetchValue(Object self) {
    try {
      ...
      throw new IllegalStateException(
          "Error while invoking " + method + " on " + self + ": " + e.getCause().getMessage(), e.getCause());
