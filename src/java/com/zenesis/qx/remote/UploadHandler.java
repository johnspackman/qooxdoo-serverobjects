...
/**
 * Handles file uploads and attaches them to a ProxySessionTracker. NOTE:: The
 * ProxySessionTracker.getBootstrapObject() MUST BE an instance of
 * FileApiProvider
 * 
...
public class UploadHandler {
  ...
  public void processUpload(HttpServletRequest request, HttpServletResponse response)
    ...
    if (contentType == null ||
        (!contentType.startsWith("multipart/form-data") && !contentType.equals("application/octet-stream"))) {
    ...
    if (maxUploadSize > -1 && request.getContentLength() > maxUploadSize) {
      ...
      response.setStatus(HttpServletResponse.SC_NOT_ACCEPTABLE,
          "Upload is too big: " + request.getContentLength() + " exceeds " + maxUploadSize);
      return;
    }
   ...
   * @throws IOException
   */
  protected void receiveMultipart(HttpServletRequest request, HttpServletResponse response,
  ...
  /**
   * Helper function which converts a query string into a map of parameters
   * 
   * @param query
   * @return
   */
