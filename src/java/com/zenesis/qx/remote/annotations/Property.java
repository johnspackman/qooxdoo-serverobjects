...
/**
 * Defines a single property; if an non-empty eventName is given then an
 * on-change event is thrown with that name, where the event is deferred and
 * asynchronous. To customise the event, @see <code>PropertyEx</code> and
 * <code>Event</code>.
 * 
 * For any given property use either <code>Property</code> or
 * <code>PropertyEx</code>, not both.
 * 
...
public @interface Property {
  ...
  /**
   * Properties in a group are treated as on demand and sent together, ie if one
   * property from that group is sent then all others in that same group are also
   * sent
   */
  ...
  /**
   * The name of the method to use to serialize the property value for sending to
   * the server
   * 
   * @return
   */
  ...
  /**
   * Whether the client should automatically create this property in it's
   * constructor, when the client creates an instance. This is only appropriate
   * for arrays and maps
   * 
   * @return
   */
