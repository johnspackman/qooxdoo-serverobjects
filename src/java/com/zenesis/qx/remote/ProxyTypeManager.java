...
/**
 * This class is responsible for creating JSON proxy definitions for the client
 * based on reflection.
 * 
 * ProxyManager only generates proxy implementations for interfaces which extend
 * from Proxied - they don't have to derive directly, but they must derive at
 * some point.
 * 
 * Because Javascript does not support overloaded methods the interfaces cannot
 * have conflicting method names; if this is the case an exception will be
 * thrown.
 * 
...
public class ProxyTypeManager {
  ...
  /**
   * Registers a class for proxying; normal rules for registration apply if
   * factory is null but otherwise factory is allowed to include methods and
   * classes that would normally be excluded
   * 
   * @param clazz
   * @param factory
   * @return
  ...
  public ProxyType getProxyType(Class clazz, ProxyTypeFactory factory) {
    ...
    synchronized (this) {
      ...
      if (clazz != Proxied.class) {
        // Get a list of ProxyTypes for interfaces that this class/interface implement
        // directly;
      ...
      // If it's an interface then there is nothing more to do except create and store
      // the ProxyType
      if (clazz.isInterface()) {
        // Poor inheritance structure may mean we've already created the ProxyType for
        // this
        // interface
        type = proxyTypes.get(clazz);
        if (type == null) {
  ...
  /**
   * Creates an instance of ProxyType; tries the factory first, then looks for
   * annotations about the factory, and falls back to default ProxyTypeImpl
   * action. This method is not expected to do anything other than create the
   * ProxyType because it is intended to be overridden if required.
   * 
   ...
   * @param interfaces a list of ProxyType's for interfaces implemented by the
   *                   clazz
   * @return
   */
  protected ProxyType newProxyType(ProxyTypeFactory factory, ProxyType superType, Class clazz,
      Set<ProxyType> interfaces) {
    ProxyType type = null;
    ...
    if (type == null) {
      com.zenesis.qx.remote.annotations.Proxied ann = getAnnotation(clazz,
          com.zenesis.qx.remote.annotations.Proxied.class);
    ...
    if (factory == null) {
      try {
        ...
        throw new IllegalArgumentException(
            "Cannot create factory " + clazz + ": " + e.getClass() + ": " + e.getMessage());
  ...
  /**
   * Tiny helper method to get an annotation (uses generics to reduce the text
   * typed in code)
   * 
   * @param <T>
   * @param clazz
