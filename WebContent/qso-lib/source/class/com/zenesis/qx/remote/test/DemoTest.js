...
qx.Class.define("com.zenesis.qx.remote.test.DemoTest", {
      ...
      map.addListener(
        ...
        function (evt) {
          ...
          if (data.type == "put") {
            ...
            this.assertEquivalent([{ key: "alpha", value: "one", entry: undefined }], data.values);
          }
        },
        this
      );
    ...
    assertEquivalent: function (expected, actual, msg) {
      if (qx.lang.Type.isArray(expected)) {
        ...
        for (var name in Object.keys(expected)) {
          if (expected[name] !== undefined) this.assertEquivalent(expected[name], actual[name], msg);
        }
      } else this.assertEquals(expected, actual, msg);
    }
  }
});
